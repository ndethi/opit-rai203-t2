...
For comparison, we also evaluated the verification performance of our ResNet18 model trained with contrastive loss, as shown in Figure \ref{fig-roc-curve-contrastive}. With an AUC of 0.98, the contrastive loss model still demonstrates excellent verification capabilities, though slightly below the performance of triplet loss (AUC = 0.99). This slight difference may be attributed to triplet loss's ability to consider the relationships between three images simultaneously (anchor, positive, and negative), providing a more comprehensive understanding of the similarity space compared to contrastive loss's pairwise approach.

![ROC curve for ResNet18 model with contrastive loss showing AUC of 0.98. While still excellent, the contrastive loss performs slightly below triplet loss in verification tasks.](figures/roc_curve_contrastive_loss.png){#fig-roc-curve-contrastive}
...
![t-SNE visualization of few-shot embeddings showing clear clustering of the five held-out breeds (Bengal, Birman, Boxer, Russian Blue, Staffordshire Bull Terrier). Note the logical organization where cat breeds cluster separately from dog breeds while maintaining their own distinct clusters. (Actual model output)](figures/tsne-fewshot.png){#fig-tsne-fewshot}
...
## Appendix A: Repository Information and Code Access

All code and materials for this project are publicly available on GitHub at the following repository:

**Repository URL**: [https://github.com/ndethi/opit-rai203-t2/tree/main/rai-8002-cv/assessment2](https://github.com/ndethi/opit-rai203-t2/tree/main/rai-8002-cv/assessment2)

The repository is organized into two main folders:

- **code/**: Contains the source code for the project
    - Charles_Watson_Ndethi_Kibaki-Code.ipynb: Main notebook that runs the complete metric learning pipeline
    - Charles_Watson_streamlit_pet_similarity_app: Script for running the interactive Streamlit demonstration

- **report/**: Contains the Quarto report and related assets
    - Charles_Watson_Ndethi_Kibaki-Report.qmd: Source for this report
    - references.bib: Bibliography file
    - ieee.csl: Citation style file
    - **figures/**: Generated figures used in the report
        - gradcam_abyssinian.png
        - roc_curve_resnet18_triplet.png
        - roc_curve_contrastive_loss.png
        - training_val_loss.png
        - tsne_fewshot.png
        - tsne_test_embeddings.png


## Appendix B: Environment Setup and Reproduction Instructions

To reproduce the results presented in this report, follow these steps:

1. **Clone the Repository**:
   ```bash
   git clone https://github.com/ndethi/opit-rai203-t2.git
   cd opit-rai203-t2/rai-8002-cv/assessment2
   ```
...
3. **Download the Dataset**:
   Run the dataset download script to fetch the Oxford-IIIT Pet Dataset:
   ```bash
   python src/download_dataset.py
   ```

4. **Train the Model**:
   Execute the training script to train the metric learning model:
   ```bash
   python src/train.py --config configs/train_config.yaml
   ```

5. **Evaluate the Model**:
   Run the evaluation script to compute verification, retrieval, and few-shot classification metrics:
   ```bash
...
6. **Generate the Report**:
   ...
   ```bash
   quarto render report/Charles_Watson_Ndethi_Kibaki-Report.qmd --to pdf
   ```
