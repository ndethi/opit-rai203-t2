...
# Create a graph structure for the AI applications
set.seed(42)
...
edges <- data.frame(
  from = c(1, 1, 4, 4, 7, 7, 10, 10, 3, 9),
  to = c(4, 2, 5, 6, 8, 9, 11, 12, 6, 12)
)
...
ggraph(graph, layout = "fr") +
  geom_edge_link(alpha = 0.4, width = 0.8) +
  geom_node_point(aes(size = size, color = category), alpha = 0.9) +
  ...
  labs(
    ...
    subtitle = "Connected Use Cases in Automotive Manufacturing"
  ) +
  theme_void() +
  theme(
    legend.position = "bottom",
...
- The network diagram shows interconnections between these applications
:::
...
compute_long <- compute_long %>%
  group_by(location) %>%
  mutate(id = row_number()) %>%
  ungroup()

# Add first point to close the polygon
compute_long_closed <- compute_long %>%
  group_by(location) %>%
  slice(c(1:n(), 1)) %>%
  ungroup()

# Calculate coordinates for radar chart
n_metrics <- length(unique(compute_data$metric))
compute_long_closed <- compute_long_closed %>%
  mutate(
    angle = 2 * pi * (id - 1) / n_metrics,
    x = score * sin(angle),
    y = score * cos(angle)
  )

# Plot
ggplot(compute_long_closed, aes(x = x, y = y, group = location, color = location)) +
  geom_path(size = 1.5) +
  geom_point(size = 3) +
  scale_color_manual(values = c("cloud" = "#4287f5", "edge" = "#42f584")) +
  coord_fixed() +
  theme_void() +
  # Add metric labels
  geom_text(
    data = compute_long_closed %>% filter(location == "cloud", id <= n_metrics),
    aes(label = metric, x = 1.2 * 10 * sin(angle), y = 1.2 * 10 * cos(angle)),
    fontface = "bold",
    hjust = ifelse(sin(compute_long_closed$angle[1:n_metrics]) > 0, 0, 1),
    vjust = ifelse(cos(compute_long_closed$angle[1:n_metrics]) > 0, 0, 1)
  ) +
  labs(
    title = "Cloud vs. Edge Capabilities",
    subtitle = "Performance Metrics (1-10 Scale)",
    color = "Computing Location"
  ) +
  theme(
...
- Radar chart shows the complementary nature of these paradigms
...
# Create a graphviz diagram for the PoC
grViz("
  digraph poc {
    graph [rankdir = LR, overlap = false, fontsize = 10, fontname = 'Arial']
    
    # Nodes
    node [shape = box, style = filled, fontname = 'Arial', fontsize = 10]
    
    Camera [label = 'Industrial\\nCamera', fillcolor = lightgrey]
    Jetson [label = 'NVIDIA Jetson\\nXavier NX', fillcolor = '#42f584']
    CloudTraining [label = 'AWS SageMaker\\nModel Training', fillcolor = '#4287f5']
    CloudStorage [label = 'AWS S3\\nDefect Storage', fillcolor = '#4287f5']
    CloudAnalytics [label = 'AWS QuickSight\\nAnalytics', fillcolor = '#4287f5']
    Alert [label = 'Alert\\nSystem', fillcolor = lightgrey]
    
    # Edges
    edge [color = gray50]
    Camera -> Jetson [label = ' Image Feed']
    Jetson -> Alert [label = ' Defect\\n Detection']
    Jetson -> CloudStorage [label = ' Defect\\n Metadata']
    CloudStorage -> CloudAnalytics [label = ' Analytics']
    CloudTraining -> Jetson [label = ' Model\\n Updates']
    CloudStorage -> CloudTraining [label = ' Training\\n Data']
    
    # Subgraphs
    subgraph cluster_edge {
      label = 'Edge Environment'
      style = dashed
      color = '#42f584'
      Camera Jetson Alert
    }
    ...
    subgraph cluster_cloud {
      ...
      style = dashed
      color = '#4287f5'
      CloudTraining CloudStorage CloudAnalytics
    }
  }
")
```
...
# Create a word cloud-like visualization for key terms
...
set.seed(42)
ggplot(key_terms, aes(label = term, size = freq, color = freq)) +
  geom_text_wordcloud(rm_outside = TRUE) +
  scale_size_area(max_size = 12) +
  scale_color_gradient(low = "#4287f5", high = "#f542cb") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5)
  )
