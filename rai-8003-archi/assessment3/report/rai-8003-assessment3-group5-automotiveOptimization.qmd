...
# Create data for PoC results
poc_results <- data.frame(
  Metric = c("Inference Time (ms)", "Throughput (FPS)", "Model Precision (%)", 
             "Model Recall (%)", "False Positive Rate (%)", "Defect Escape Reduction (%)"),
  Achieved = c(27.3, 36.8, 98.7, 97.4, 1.2, 58.0),
  Target = c(30.0, 30.0, 98.0, 97.0, 1.5, 40.0)
)

# Calculate achievement percentage for color coding
poc_results$Achievement <- ifelse(
  poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)"),
  poc_results$Target / poc_results$Achieved, # Lower is better
  poc_results$Achieved / poc_results$Target  # Higher is better
)

# Create achievement labels
poc_results$Label <- ifelse(
  poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)"),
  paste0(round((1 - poc_results$Achieved / poc_results$Target) * 100, 1), "% better"),
  paste0(round((poc_results$Achieved / poc_results$Target - 1) * 100, 1), "% better")
)

# For metrics where lower is better, we need to adjust
poc_results$Label[poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)")] <- 
  ifelse(
    poc_results$Achieved[poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)")] < 
    poc_results$Target[poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)")],
    poc_results$Label[poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)")],
    paste0(round((poc_results$Achieved[poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)")] / 
                  poc_results$Target[poc_results$Metric %in% c("Inference Time (ms)", "False Positive Rate (%)")] - 1) * 100, 1), "% worse")
  )

# Create the plot
ggplot(poc_results, aes(x = reorder(Metric, Achievement))) +
  geom_segment(aes(y = 0, yend = Achieved, xend = Metric), color = "grey50") +
  geom_point(aes(y = Achieved), color = "#4287f5", size = 10, alpha = 0.7) +
  geom_point(aes(y = Target), color = "#f5a742", shape = 18, size = 5) +
  geom_text(aes(y = Achieved, label = Label),
  ...
  labs(
    ...
    subtitle = "Achieved metrics vs. targets (◆ = target value)"
  ) +
  theme_minimal() +
  theme(
...
#| fig-cap: "Business Impact of Hybrid Cloud-Edge Architecture"
#| warning: false
#| echo: false
#| message: false

# Create data for business impact
impact_data <- data.frame(
  Category = c("Quality", "Operations", "Technology", "Financial"),
  Metric = c("Defect Escape Rate", "Production Line Throughput", "Network Bandwidth", "Cloud Computing Costs"),
  Improvement = c(-58, 22, -95.7, -73.6),
  Direction = c("decrease", "increase", "decrease", "decrease")
)

# Set colors based on whether increase or decrease is positive
impact_data$Color <- ifelse(
  impact_data$Direction == "decrease",
  "#4287f5", "#42f584"
)

# Create labels with appropriate signs
impact_data$Label <- ifelse(
  impact_data$Direction == "decrease",
  paste0(abs(impact_data$Improvement), "% reduction"),
  paste0(impact_data$Improvement, "% increase")
)

# Create the plot
ggplot(impact_data, aes(x = Category, y = abs(Improvement), fill = Color)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_text(aes(label = Label, y = abs(Improvement) + 5),
...
phases <- data.frame(
  Phase = c("1. Infrastructure Audit", "2. Use Case Selection", "3. PoC Development", 
            "4. Pilot Rollout", "5. Enterprise Deployment"),
  Start = c(1, 2, 3, 5, 8),
  End = c(2, 3, 5, 8, 18),
  Key_Milestone = c("Baseline Assessment", "Use Case Prioritization", 
                   "Working Prototype", "Validated Results", "Full Implementation")
)

# Create milestones data
milestones <- data.frame(
  Phase = phases$Phase,
  x = c(2, 3, 5, 8, 18),
  y = 1:5,
  Milestone = phases$Key_Milestone
)

# Create the Gantt chart
ggplot() +
  # Add the phase bars
  geom_segment(data = phases, 
               aes(x = Start, xend = End, y = factor(Phase, levels = rev(Phase)), yend = factor(Phase, levels = rev(Phase))),
               size = 10, color = "#4287f5", alpha = 0.7) +
  # Add milestone markers
  geom_point(data = milestones, 
             aes(x = x, y = factor(Phase, levels = rev(Phase))),
             size = 5, color = "#f5a742") +
  geom_text(data = milestones,
            aes(x = x, y = factor(Phase, levels = rev(Phase)), label = Milestone),
            hjust = -0.1, vjust = -1.5, size = 3) +
  # Add month labels
  scale_x_continuous(breaks = seq(0, 18, by = 2),
                    labels = paste0("M", seq(0, 18, by = 2))) +
  labs(
    x = "Timeline (Months)",
    y = NULL,
  ...
  # Extend x-axis to show milestone labels
